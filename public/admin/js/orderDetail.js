...
const displayHeader = (id) => {
  ...
  header.querySelector("h1").textContent = `Order ${
    id.length === 1 ? `0${id}` : id
  }`;
  return header;
};
...
const displayShipping = (orderObject) => {
  ...
  section.querySelector(".user__cell a").href = `tel:+92${orderObject.tel.slice(
    1
  )}`;
  section.querySelector(".user__address span").textContent =
    orderObject.address;
...
const displayItem = (item) => {
  ...
  li.querySelector(".item__price").textContent = `${
    item.qty
  } x Rs. ${numberWithCommas(item.price)} = Rs. ${numberWithCommas(
    item.total
  )}`;
  return li;
};
...
const displaySummary = (order) => {
  ...
  section.querySelector(
    ".order__total span"
  ).textContent = `Rs. ${numberWithCommas(order.subtotal)}`;
  section.querySelector(".order__discount-code span").textContent =
    order.discountCode;
  section.querySelector(".order__discount span").textContent =
    order.discountPercent;
  section.querySelector(
    ".order__final-price span"
  ).textContent = `Rs. ${numberWithCommas(order.totalPrice)}`;
