...
import { createElementWithOptions } from "../../js/dataFunctions.js";

const productId = new URLSearchParams(window.location.search).get("id");

const init = async () => {
  let product = await getDataFromDb(`product/${productId}`);
...
const createImageGrid = ({ name, primary_image, img: images }) => {
  primary_image = `http://127.0.0.1:8000${primary_image}`;
  images =
    images.length > 0
      ? images.map((img) => `http://127.0.0.1:8000${img.url}`)
      : [primary_image];
  const imageGrid = createElementWithOptions("div", ["image-grid"]);
  imageGrid.appendChild(createImage(primary_image, name, "primary__image"));
...
const createProductDetails = (product) => {
  const productDetails = createElementWithOptions("div", [
    "product__details",
    "flow",
  ]);
...
const displayProductDescription = (description) => {
  const p = createElementWithOptions(
    "p",
...
const createEditBtn = (id) => {
  const btn = createElementWithOptions(
    "a",
    ["btn", "edit-product"],
    "Edit Product..."
  );
...
const createReviewsView = (reviews) => {
  const reviewsSection = createElementWithOptions("section", [
    "reviews",
    "container",
  ]);
  ...
  if (reviews.length) {
    const reviewItems = reviews.map((review) =>
      createReviewItem(review, reviews)
    );
